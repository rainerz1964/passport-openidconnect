 ...
 */
 ...
 // property to pass the code_verifier needed for PKCE based authentication

Strategy.prototype._verifier = '';
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    function loaded(err, ok, state) {
      ...
      authenticationParams['grant_type'] = 'authorization_code';
      authenticationParams['redirect_uri'] = callbackURL;
      if (self._pkce) {
        authenticationParams['code_verifier'] = this._verifier;
      }
      
      var oauth2 = self._getOAuth2Client(meta);

      oauth2.getOAuthAccessToken(code, authenticationParams, function(err, accessToken, refreshToken, params) {
    ...
    this._setup(identifier, function(err, config) {
      ...
      if (self._pkce) {
        self._verifier = utils.base64URLEncode(utils.randomBytes(32));
        this._verifier = self._verifier;
        params['code_challenge'] = utils.base64URLEncode(utils.sha256(self._verifier));
      ...
      if (!config.nonce && self._pkce) { params.nonce = utils.uid(20); }
